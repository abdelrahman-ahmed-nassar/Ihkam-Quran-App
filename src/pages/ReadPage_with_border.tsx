...
function getMiddleFramePath() {
  return `${import.meta.env.BASE_URL}images/middle.png`;
}
...
const ReadPage = ({
  ...
  // Compute font-size so every line fits within the framed page area.
  // Each line occupies (fontSize × LINE_HEIGHT_RATIO) px vertically.
  // Reserve room for the floating button + decorative frame margins.
  const LINE_HEIGHT_RATIO = 2.08;
  const lineCount = Math.max(pageLines.length, 1);
  const dynamicFontSize = `min(calc((100dvh - 6.5rem) / ${lineCount * LINE_HEIGHT_RATIO}), calc((100vw - 3rem) / 18.6))`;
    ...
    <div className="fixed inset-0 overflow-hidden bg-background">
      ...
      {wordsMap && isPageFontReady && !errorMessage && (
        ...
        >
          <div
            ref={pageContentRef}
            className="mx-auto w-max max-w-full bg-no-repeat px-[0.9em] py-[1.1em] [unicode-bidi:embed] will-change-transform"
            style={{
              fontFamily: getQpcFontStack(currentPage),
              fontSize: dynamicFontSize,
              lineHeight: LINE_HEIGHT_RATIO,
              backgroundImage: `url("${getMiddleFramePath()}")`,
              backgroundSize: "100% 100%",
              backgroundPosition: "center",
            }}
          >
            <div className="px-[0.8em] py-[0.3em]">
